---
title: "Predict Externalizing Symptoms for Children"
...
Predicts externalizing symptoms using XGBoost with MRI, demographics, and SES features. Includes stratified splitting, mean imputation, hyperparameter tuning, and visualizations of model performance and tuning results.
...
bpm <- read.csv("/Users/maggieheimvik/Desktop/GitHub/masters_thesis/Data/core/mental-health/mh_y_bpm.csv") %>%
  ...
  select(src_subject_id, bpm_externalizing)
...
# Remove rows with missing bpm_externalizing
full_dat <- full_dat %>%
  filter(!is.na(bpm_externalizing))
```
...
# Create stratified split indices for bpm_externalizing
train_index <- createDataPartition(full_dat$bpm_externalizing, p = 0.7, list = FALSE)
...
test_index <- createDataPartition(remaining_data$bpm_externalizing, p = 0.5, list = FALSE)
...
cat("Full dataset:\n")
print(table(full_dat$bpm_externalizing))

cat("\nTraining set:\n")
print(table(train_data$bpm_externalizing))

cat("\nCalibration set:\n")
print(table(calib_data$bpm_externalizing))

cat("\nTest set:\n")
print(table(test_data$bpm_externalizing))
...
x_train <- model.matrix(bpm_externalizing ~ ., train_data %>% select(-all_of(cols_to_remove)))[, -1]
x_calib <- model.matrix(bpm_externalizing ~ ., calib_data %>% select(-all_of(cols_to_remove)))[, -1]
x_test  <- model.matrix(bpm_externalizing ~ ., test_data  %>% select(-all_of(cols_to_remove)))[, -1]


#target variables
y_train <- train_data$bpm_externalizing
y_calib <- calib_data$bpm_externalizing
y_test  <- test_data$bpm_externalizing
...
# Save results to CSV
write.csv(results_df, "bpm_external_hyperparameter_tuning_results.csv", row.names = FALSE)
```
...
Make predictions
```{r}
#results_df <- read.csv("/Users/maggieheimvik/Desktop/GitHub/masters_thesis/Data/ocd//outputs/models/multiclass_cbcl_bpm/bpm_external_hyperparameter_tuning_results.csv")
...
```{r}
#save model
xgb.save(final_model,  fname = "/Users/maggieheimvik/Desktop/GitHub/masters_thesis/Data/ocd/outputs/models/multiclass_cbcl_bpm/m_bpm_e_model.model")

#save everything together
save(final_model, x_test, y_test, x_calib, y_calib, pred_probs, pred_classes_argmax, train_data, test_data, calib_data, dcalib,
     file = "/Users/maggieheimvik/Desktop/GitHub/masters_thesis/Data/ocd/outputs/models/multiclass_cbcl_bpm/m_bpm_e_model+preds.RData")
```
...
#ggsave("optimization_history_bpm_i.png", plot = p2, width = 10, height = 6, dpi = 300)
