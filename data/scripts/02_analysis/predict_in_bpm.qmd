---
title: "Predict Internalizing Symptoms for Child-Report"
...
Predicts internalizing symptoms (BPM) using XGBoost with MRI data. Includes stratified data splitting, class weighting, hyperparameter tuning, and visualization of model optimization and performance.
...
bpm <- read.csv("/Users/maggieheimvik/Desktop/GitHub/masters_thesis/Data/core/mental-health/mh_y_bpm.csv") %>%
  ...
  select(src_subject_id, bpm_internalizing)
...
# Create stratified split indices for bpm_internalizing
...
cat("Full dataset:\n")
print(table(full_dat$bpm_internalizing))

cat("\nTraining set:\n")
print(table(train_data$bpm_internalizing))

cat("\nCalibration set:\n")
print(table(calib_data$bpm_internalizing))

cat("\nTest set:\n")
print(table(test_data$bpm_internalizing))
...
x_train <- model.matrix(bpm_internalizing ~ ., train_data %>% select(-all_of(cols_to_remove)))[, -1]
x_calib <- model.matrix(bpm_internalizing ~ ., calib_data %>% select(-all_of(cols_to_remove)))[, -1]
x_test  <- model.matrix(bpm_internalizing ~ ., test_data  %>% select(-all_of(cols_to_remove)))[, -1]


#target variables
y_train <- train_data$bpm_internalizing
y_calib <- calib_data$bpm_internalizing
y_test  <- test_data$bpm_internalizing
...
# Save results to CSV
write.csv(results_df, "bpm_internal_hyperparameter_tuning_results.csv", row.names = FALSE)
...
Make predictions
```{r}
#results_df <- read.csv("/Users/maggieheimvik/Desktop/GitHub/masters_thesis/Data/ocd//outputs/models/multiclass_cbcl_bpm/bpm_internal_hyperparameter_tuning_results.csv")
#xgboost test matrix 
...
# Get argmax predictions (standard way) thresholds = 0.5
...
```{r}
#save model
xgb.save(final_model, 
         fname = "/Users/maggieheimvik/Desktop/GitHub/masters_thesis/Data/ocd/outputs/models/multiclass_cbcl_bpm/m_bpm_i_model.model")

#save everything together
save(final_model, x_test, y_test, x_calib, y_calib, pred_probs, pred_classes_argmax, train_data, test_data, calib_data, dcalib,
     file = "/Users/maggieheimvik/Desktop/GitHub/masters_thesis/Data/ocd/outputs/models/multiclass_cbcl_bpm/m_bpm_i_model+preds.RData")
```


# Plor hyper parameters 
```{r}
