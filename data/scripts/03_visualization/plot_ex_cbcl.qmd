---
title: "Plot Externalizing Symptoms for Parent-Report"
...
Evaluates XGBoost multiclass model for CBCL externalizing symptoms using argmax and threshold-calibrated classification. Includes confusion matrices, OvR ROC curves, class-wise probability histograms, and performance visualization on test data.

# load model
```{r}
load("/Users/maggieheimvik/Desktop/GitHub/masters_thesis/Data/ocd/outputs/models/multiclass_cbcl_bpm/m_cbcl_e_model+preds.RData")
```
...
plots <- lapply(class_labels, function(cls) {
  ...
  roc_plot <- ggplot(roc_df, aes(x = FPR, y = TPR)) +
    ...
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "#2E8B57") +
    labs(
...
ggsave("cbcl_e_OvR_ROC.png", final_plot, width = 12, height = 6, dpi = 300)
```
