---
title: "Plot Internalizing Symptoms for Parent-Report"
...
Evaluates the XGBoost model predicting CBCL internalizing symptoms. Includes argmax and threshold-calibrated accuracy, ROC and histogram plots per class, confusion matrix heatmap, calibration via Youden's J, and a permutation test for significance.
...
ggsave("cbcl_i_OvR_ROC.png", final_plot, width = 12, height = 6, dpi = 300)
```
...
perm_plot <- ggplot(hist_data, aes(x = stat)) +
  geom_density(fill = "#2E8B57", alpha = 0.4) +
  geom_vline(xintercept = observed_acc, color = "blue", size = 1.2) +
  annotate("text", x = observed_acc, y = y_pos,
           label = "Observed accuracy", hjust = -0.1, vjust = 1, color = "blue", size = 4) +
  labs(
